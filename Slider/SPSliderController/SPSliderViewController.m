...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers
{
    ...
    if (self) {
        ...
        _segmentBarHeight = 45.0f;//之前设置的
//        _segmentBarHeight = 64.0f;
...
- (void)viewDidLoad
{
    ...
    self.automaticallyAdjustsScrollViewInsets = NO;//自己测试
...
#warning 自己变的的让其点击同一个有同样的效果
    
    if (self.presentViewIndex == indexPath.row) {
        if ([_delegate respondsToSelector:@selector(slideViewController:didSelectViewIndex:)]) {
            [_delegate slideViewController:self didSelectViewIndex:indexPath.row];
        }
        return;
    }
...
#warning 把这里关了的话。滑动的时候SPMenuCategoryViewController 代理方法不会走
        if ([_delegate respondsToSelector:@selector(slideViewController:didSelectViewIndex:)]) {
...
- (UIView *)indicator
{
    if (!_indicator) {
        ...
        _indicator.backgroundColor = [UIColor colorWithHexString:@"FF3300"];
    }
    return _indicator;
}
